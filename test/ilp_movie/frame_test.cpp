...
TEST_CASE("CompPixelData")
{
  ...
  SECTION("rgb")
  {
    ...
    const auto r = CompPixelData<float>(f.data, f.linesize, Comp::kR, h, f.hdr.pix_fmt_name);
    const auto g = CompPixelData<float>(f.data, f.linesize, Comp::kG, h, f.hdr.pix_fmt_name);
    const auto b = CompPixelData<float>(f.data, f.linesize, Comp::kB, h, f.hdr.pix_fmt_name);
    const auto a = CompPixelData<float>(f.data, f.linesize, Comp::kA, h, f.hdr.pix_fmt_name);
  ...
  SECTION("rgba") {
    ...
    REQUIRE(reinterpret_cast<uint8_t *>(b.data) == f.data[1]);
    REQUIRE(reinterpret_cast<uint8_t *>(r.data) == f.data[2]);
    REQUIRE(reinterpret_cast<uint8_t *>(a.data) == f.data[3]);

    const auto line0 = static_cast<std::size_t>(f.linesize[0]);
    const auto line1 = static_cast<std::size_t>(f.linesize[1]);
    const auto line2 = static_cast<std::size_t>(f.linesize[2]);
    const auto line3 = static_cast<std::size_t>(f.linesize[3]);
    REQUIRE(g.count == h * (line0 / sizeof(float)));
    REQUIRE(b.count == h * (line1 / sizeof(float)));
    REQUIRE(r.count == h * (line2 / sizeof(float)));
    REQUIRE(a.count == h * (line3 / sizeof(float)));
  }
}
